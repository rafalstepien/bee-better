...
from habit_tracker.common import is_ajax, ColorsHEX, ColorsRGB
from habit_tracker.views.habits_view_data_extractor import HabitsViewDataExtractor
from habit_tracker.models import HabitTrack, Habit
...
def update_habit(request):
    if is_ajax(request) and request.method == "POST":
        habit_request_data = HabitsViewDataExtractor(
            request.user.id
        ).get_habit_data_from_request_body(request.body)

        habit_track_object = HabitTrack.objects.filter(
            habit_date=habit_request_data.date,
            habit__habit_name=habit_request_data.name,
            habit__user=request.user.id
        ).first()
        ...
        if not habit_track_object:
            ...
            habit_track_object = HabitTrack(
                habit=habit_object,
                habit_date=habit_request_data.date,
                habit_done=habit_request_data.value
            )
        else:
